...
import { getInventoryById } from "../../api/statisticsService";
...
const CardDetails = (props) => {
  ...
  return (
    <Paper style={{ height: "100%", width: "100%", padding: 20 }}>
      <h1>
        Thông tin hiển thị{" "}
        <Tag style={{ margin: "10px 0px", fontSize: 14 }} color={"green"}>
          {getInform()}
        </Tag>
      </h1>

      {filter.keySearch?.toLowerCase().match(/spv\d+/g) ? (
        <Tag style={{ margin: "10px 0px", fontSize: 14 }} color={"orange"}>
          <Link
            to={`/products/${
              filter.type === 1
                ? imports?.at(0)?.productId
                : filter.type === 2
                ? stokes?.at(0)?.productId
                : null
            }?backcode=statistic`}
          >
            {filter.keySearch.toUpperCase()}
          </Link>
        </Tag>
      ) : (
        <div>
          {filter.keySearch !== "" ? (
            <Tag style={{ marginBottom: 10, fontSize: 14 }} color={"yellow"}>
              {filter.keySearch}
            </Tag>
          ) : null}
        ...
        <Col span={18}>
          {filter.inventoryId > 0 ? (
            <p>
              ...
              >
                <Link to={`/stocker/inventories/${inventory?.id}`}>
                  {inventory?.code}
                </Link>
              </Tag>{" "}
            </p>
          ...
          <Col span={18}>
            {filter.supplierId > 0 ? (
              <p>
                ...
                >
                  <Link to={`/stocker/supplier/details/${supplier?.id}`}>
                    {supplier?.code}
                  </Link>
                </Tag>
              </p>
        ...
        <Col span={18}>
          ...
          <Tag color="green" style={{ fontSize: 14 }}>
            {filter.type === 1
              ? filter
                  .filter((value, index) => {
                    if (value.key === filter.sortBy) return value.value;
                  })
                  .at(0)?.value
              : filter
                  .filter((value, index) => {
