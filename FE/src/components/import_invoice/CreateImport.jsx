...
import SelectSupplier from "../../common_components/SelectSupplier";
import {
  createImport,
...
const CreateImport = () => {
  ...
  const handleAddToTable = (item) => {
    const productVariant = productVariants.find((p) => p.id === item);
    const newData = {
  ...
  const setTotal = (data, index) => {
    ...
    data[index]["totalPrice"] = Number(
      data[index]["quantity"] * data[index]["importPrice"]
    );
  };
          ...
          <Col span={8}>
            <div className="block">
              ...
              <Form.Item
                ...
                name="inventory"
                rules={[
                  { required: true, message: "Chi nhánh không được để trống" },
                ]}
              >
                <Select
                  showSearch
