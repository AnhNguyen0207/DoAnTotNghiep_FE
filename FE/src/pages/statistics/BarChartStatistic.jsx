...
const BarChartStatistic = (props) => {
  ...
  let dataChart = props.imports.filter((item) => {
    return {
      name: item.name,
  ...
  const [data, setData] = useState(dataChart.slice(0, 5));
    ...
    <div>
      <ResponsiveContainer width={"100%"} height={500}>
        ...
        >
          ...
          <XAxis dataKey={"name"} display={"none"}></XAxis>

          <YAxis />
          <Tooltip />
          <Legend />
          {visible.importNumber ? (
            <Bar
              dataKey="importNumber"
              name={"Số lượng đặt"}
              stackId={0}
              fill={COLORS[0]}
              overflow="scroll"
      ...
      <div>
        <Checkbox
          defaultChecked={visible.receiveNumber}
          onChange={(e) => {
        ...
        <Checkbox
          defaultChecked={visible.returnNumber}
          onChange={(e) => {
        ...
        <Checkbox
          defaultChecked={visible.importNumber}
          onChange={(e) => {
