...
const CreateReturnImportInvoice = () => {
    ...
    useEffect(() => {
        getImportReturn(code).then(result => {
            const newResult = result.data.map((obj) => ({...obj, inputQuantity: 0}))
    ...
    const onSubmit = () => {
        ...
        importReturn.map((obj) => {
            if (obj.inputQuantity > 0) {
                list.push({detailsImportId: obj.detailsImportId, quantity: obj.inputQuantity})
            }
        })
        ...
        returnImportInvoice(returnImport,detailInvoices?.anImport.inventoryId).then(() => {
